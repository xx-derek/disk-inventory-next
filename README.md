...
[CHANGELOG.md](CHANGELOG.md) is this fork's history. Notes inherited from the original
project live in [`documentation/`](documentation/); `release notes.txt` there is the
version history of the **original** Disk Inventory X releases and intentionally still
refers to it by that name — it is a historical record, not this project's.
